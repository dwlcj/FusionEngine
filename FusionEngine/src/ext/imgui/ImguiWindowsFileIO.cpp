...
vector<string> getWindowsDrives()
{
    ...
    else
    {
        ...
        {
            if (uDriveMask & 1)
                result.push_back( (char*)g_szDrvMsg );
        ...
        ImVec2 size )
{
    ...
    if( CollapsingHeader("Browse Directories") )
    {
...
#if defined(WIN32)
		auto drive_list = getWindowsDrives();
		...
		static const char* const listbox_items[]  = { "C:" , "D:" , "E:", "F:" , "G:" }; //hack

        if( ListBox( "  ", &drive_selected, listbox_items, drive_list.size() ) )
        {
