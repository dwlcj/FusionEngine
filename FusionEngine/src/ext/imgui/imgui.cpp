...
// VCL3D's additions start
bool ImGui::ImageTextButton(ImTextureID user_texture_id,
	const ImVec2& size, const char* label, const ImVec4& txt_col,
	const ImVec2& txt_pos,
...
)
{
	...
	//PushStyleColor(ImGuiCol_Button, txt_col);
	// Render
	const ImU32 col = GetColorU32((hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
	//PopStyleColor();
	RenderNavHighlight(bb, id);
	...
	//RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
	RenderTextClipped(bb.Min + style.FramePadding + txt_pos, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
	PopStyleColor();
...
void ImGui::ImRotateEnd(float rad, int rotation_start_index)
{
...
void ImGui::PlotHistogram(const char* label, const float* values, int values_count, ImVec2 graph_size, int values_offset, const char* overlay_text, float scale_min, float scale_max, int stride)
{
    ImGuiPlotArrayGetterData data(values, stride);
